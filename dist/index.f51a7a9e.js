function SnackBar(userOptions) {
    ...
    function _setContainer() {
        var target1 = getOrFindContainer();
        if (target1 === undefined) {
            console.warn("SnackBar: Could not find target container " + _Options.container);
            target1 = document.body; // default to the body as the container
        }
        _Container = getOrAddContainerIn(target1);
    ...
    function _applyPositionClasses() {
        ...
        if (_Options.fixed) _Container.classList.add(fixedClassName);
        else _Container.classList.remove(fixedClassName);
    }
    function _createMessage() {
        var outerElement1 = createWrapper();
        var innerSnack1 = createInnerSnackbar();
        outerElement1.appendChild(innerSnack1);
        ...
        function applyColorAndIconTo(element1) {
            if (!_Options.status) return;
            var status = document.createElement("span");
            status.classList.add("js-snackbar__status");
            applyColorTo(status);
            applyIconTo(status);
            element1.appendChild(status);
...
//# sourceMappingURL=index.f51a7a9e.js.map
